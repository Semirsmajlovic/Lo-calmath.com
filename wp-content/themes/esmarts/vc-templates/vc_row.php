...
if ( $row_content_width !== 'grid' ) {
	...
	if ( ! empty( $simple_background_image ) ) {
		...
		$eltdf_vc_row_style[] = 'background-image: url(' . esc_url( $img_src ) . ')';
	}
	...
	if ( ! empty( $simple_background_image ) ) {
		$image_src            = wp_get_attachment_image_src($simple_background_image, 'full');
		$img_src = is_array($image_src) ? $image_src[0] : '';
		$eltdf_grid_row_style[] = 'background-image: url(' . esc_url( $img_src ) . ')';
	}
...
if($row_content_width === 'grid'){
	$eltdf_row_wrapper_start .= '<div class="eltdf-row-grid-section-wrapper '.esc_attr($grid_row_classes).'" '.esc_attr($grid_row_datas).' ' . esmarts_elated_get_inline_style( $eltdf_grid_row_styles ) . '><div class="eltdf-row-grid-section">';
...
$output .= '<div ' . implode( ' ', $wrapper_attributes ) . ' ' . esmarts_elated_get_inline_style( $eltdf_vc_row_styles ) . '>';
$output .= wpb_js_remove_wpautop( $content );
$output .= '</div>';
$output .= $eltdf_row_wrapper_end;
$output .= $after_output;

echo esmarts_elated_get_module_part( $output );