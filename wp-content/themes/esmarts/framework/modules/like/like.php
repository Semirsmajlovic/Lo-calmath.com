...
class eSmartsElatedClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_esmarts_elated_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_esmarts_elated_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'esmarts' );
		
		if ( isset( $_COOKIE[ 'eltdf-like_' . $post->ID ] ) ) {
			$class = 'eltdf-like liked';
			$title = esc_html__( 'You already like this!', 'esmarts' );
		}

		return '<a href="#" class="' . esc_url( $class ) . '" id="eltdf-like-' . esc_attr( $post->ID ) . '-' . $rand_number . '" title="' . esc_attr( $title ) . '">' . $output . '</a>';
	}
}

if ( ! function_exists( 'esmarts_elated_create_like' ) ) {
	function esmarts_elated_create_like() {
		eSmartsElatedClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'esmarts_elated_create_like' );
}