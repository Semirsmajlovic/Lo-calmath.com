...
namespace eSmartsElatedNamespace\Modules\Header\Lib;
/**
 * Class eSmartsElatedClassHeaderConnector
 *
...
class eSmartsElatedClassHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'esmarts_elated_action_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'esmarts_elated_filter_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'esmarts_elated_filter_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'esmarts_elated_action_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to esmarts_elated_filter_title_content_padding filter
	 *
	 * @return int
	 */
