...
if ( ! function_exists( 'esmarts_elated_header_skin_class' ) ) {
	/**
	 * Function that adds header style class to body tag
	 */
	function esmarts_elated_header_skin_class( $classes ) {
		$header_style     = esmarts_elated_get_meta_field_intersect( 'header_style', esmarts_elated_get_page_id() );
		$header_style_404 = esmarts_elated_options()->getOptionValue( '404_header_style' );
	...
	add_filter( 'body_class', 'esmarts_elated_header_skin_class' );
}

if ( ! function_exists( 'esmarts_elated_sticky_header_behaviour_class' ) ) {
	/**
	 * Function that adds header behavior class to body tag
	 */
	function esmarts_elated_sticky_header_behaviour_class( $classes ) {
		$header_behavior = esmarts_elated_get_meta_field_intersect( 'header_behaviour', esmarts_elated_get_page_id() );
	...
	add_filter( 'body_class', 'esmarts_elated_sticky_header_behaviour_class' );
}

if ( ! function_exists( 'esmarts_elated_menu_dropdown_appearance' ) ) {
	/**
	 ...
	 */
	function esmarts_elated_menu_dropdown_appearance( $classes ) {
		$dropdown_menu_appearance = esmarts_elated_options()->getOptionValue( 'menu_dropdown_appearance' );
	...
	add_filter( 'body_class', 'esmarts_elated_menu_dropdown_appearance' );
}

if ( ! function_exists( 'esmarts_elated_header_class' ) ) {
	/**
	 ...
	 */
	function esmarts_elated_header_class( $classes ) {
		$id = esmarts_elated_get_page_id();
		
		$header_type = esmarts_elated_get_meta_field_intersect( 'header_type', $id );
		
		$classes[] = 'eltdf-' . $header_type;
		
		$disable_menu_area_shadow = esmarts_elated_get_meta_field_intersect( 'menu_area_shadow', $id ) == 'no';
		if ( $disable_menu_area_shadow ) {
			$classes[] = 'eltdf-menu-area-shadow-disable';
		}
		
		$disable_menu_area_grid_shadow = esmarts_elated_get_meta_field_intersect( 'menu_area_in_grid_shadow', $id ) == 'no';
		if ( $disable_menu_area_grid_shadow ) {
			$classes[] = 'eltdf-menu-area-in-grid-shadow-disable';
		}
		
		$disable_menu_area_border = esmarts_elated_get_meta_field_intersect( 'menu_area_border', $id ) == 'no';
		if ( $disable_menu_area_border ) {
			$classes[] = 'eltdf-menu-area-border-disable';
		}
		
		$disable_menu_area_grid_border = esmarts_elated_get_meta_field_intersect( 'menu_area_in_grid_border', $id ) == 'no';
		if ( $disable_menu_area_grid_border ) {
			$classes[] = 'eltdf-menu-area-in-grid-border-disable';
		}
		
		if ( esmarts_elated_get_meta_field_intersect( 'menu_area_in_grid', $id ) == 'yes' &&
		     esmarts_elated_get_meta_field_intersect( 'menu_area_grid_background_color', $id ) !== '' &&
		     esmarts_elated_get_meta_field_intersect( 'menu_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'eltdf-header-menu-area-in-grid-padding';
		}
		
		$disable_logo_area_border = esmarts_elated_get_meta_field_intersect( 'logo_area_border', $id ) == 'no';
		if ( $disable_logo_area_border ) {
			$classes[] = 'eltdf-logo-area-border-disable';
		}
		
		$disable_logo_area_grid_border = esmarts_elated_get_meta_field_intersect( 'logo_area_in_grid_border', $id ) == 'no';
		if ( $disable_logo_area_grid_border ) {
			$classes[] = 'eltdf-logo-area-in-grid-border-disable';
		}
		
		if ( esmarts_elated_get_meta_field_intersect( 'logo_area_in_grid', $id ) == 'yes' &&
		     esmarts_elated_get_meta_field_intersect( 'logo_area_grid_background_color', $id ) !== '' &&
		     esmarts_elated_get_meta_field_intersect( 'logo_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'eltdf-header-logo-area-in-grid-padding';
		}
		
		$disable_shadow_vertical = esmarts_elated_get_meta_field_intersect( 'vertical_header_shadow', $id ) == 'no';
		if ( $disable_shadow_vertical ) {
			$classes[] = 'eltdf-header-vertical-shadow-disable';
		}
		
		$disable_border_vertical = esmarts_elated_get_meta_field_intersect( 'vertical_header_border', $id ) == 'no';
	...
	add_filter( 'body_class', 'esmarts_elated_header_class' );
}

if ( ! function_exists( 'esmarts_elated_header_area_style' ) ) {
	/**
	 * Function that return styles for header area
	 */
	function esmarts_elated_header_area_style( $style ) {
		...
		$menu_area_background_color_rgba = esmarts_elated_rgba_color( $menu_area_background_color, $menu_area_background_transparency );
		...
		$menu_area_grid_background_color_rgba = esmarts_elated_rgba_color( $menu_area_grid_background_color, $menu_area_grid_background_transparency );
		
		if ( $menu_area_grid_background_color_rgba !== null ) {
			$menu_area_grid_style['background-color'] = $menu_area_grid_background_color_rgba;
		}
		
		$current_style .= esmarts_elated_dynamic_css( $menu_area_selector, $menu_area_style );
		$current_style .= esmarts_elated_dynamic_css( $menu_area_grid_selector, $menu_area_grid_style );
		...
		if ( $dropdown_top_position !== '' ) {
			$dropdown_styles['top'] = esmarts_elated_filter_suffix( $dropdown_top_position, '%' ) . '%';
		}
		
		$dropdown_selector = array( $class_prefix . ' .eltdf-page-header .eltdf-drop-down .second' );
		
		$current_style .= esmarts_elated_dynamic_css( $dropdown_selector, $dropdown_styles );
		...
		$logo_area_background_color_rgba = esmarts_elated_rgba_color( $logo_area_background_color, $logo_area_background_transparency );
		...
		$logo_area_grid_background_color_rgba = esmarts_elated_rgba_color( $logo_area_grid_background_color, $logo_area_grid_background_transparency );
		...
		if ( ! empty( $logo_area_style ) ) {
			$current_style .= esmarts_elated_dynamic_css( $logo_area_selector, $logo_area_style );
		}
		
		if ( ! empty( $logo_area_grid_style ) ) {
			$current_style .= esmarts_elated_dynamic_css( $logo_area_grid_selector, $logo_area_grid_style );
		}
		
		$current_style = apply_filters( 'esmarts_elated_filter_add_header_page_custom_style', $current_style, $class_prefix, $page_id ) . $style;
		
		return $current_style;
	}
	
	add_filter( 'esmarts_elated_filter_add_page_custom_style', 'esmarts_elated_header_area_style' );
}