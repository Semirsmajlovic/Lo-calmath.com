...
class eSmartsElatedClassIconWidget extends eSmartsElatedClassWidget {
	public function __construct() {
		parent::__construct(
			'eltdf_icon_widget',
			esc_html__( 'Elated Icon Widget', 'esmarts' ),
			array( 'description' => esc_html__( 'Add icons to widget areas', 'esmarts' ) )
		);
	...
	protected function setParams() {
		$this->params = array_merge(
			esmarts_elated_icon_collections()->getIconWidgetParamsArray(),
			array(
				array(
					'type'  => 'textfield',
					'name'  => 'icon_text',
					'title' => esc_html__( 'Icon Text', 'esmarts' )
				),
				array(
					'type'  => 'textfield',
					'name'  => 'link',
	...
	public function widget( $args, $instance ) {
		...
		$data   = array();
		$data[] = ! empty( $instance['icon_hover_color'] ) ? esmarts_elated_get_inline_attr( $instance['icon_hover_color'], 'data-hover-color' ) : '';
		$data   = implode( ' ', $data );
		...
		if ( ! empty( $instance['icon_size'] ) ) {
			$holder_styles[] = 'font-size: ' . esmarts_elated_filter_px( $instance['icon_size'] ) . 'px';
		}
		...
		<a <?php esmarts_elated_class_attribute( $holder_classes ); ?> <?php echo wp_kses_post( $data ); ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>" <?php echo esmarts_elated_get_inline_style( $holder_styles ); ?>>
