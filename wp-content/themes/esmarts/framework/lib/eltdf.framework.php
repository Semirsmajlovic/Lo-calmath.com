...
/*
   Class: eSmartsElatedClassFramework
...
class eSmartsElatedClassFramework {
	...
	private function __construct() {
		$this->eltdOptions         = eSmartsElatedClassOptions::get_instance();
		$this->eltdMetaBoxes       = eSmartsElatedClassMetaBoxes::get_instance();
		$this->eltdTaxonomyOptions = eSmartsElatedClassTaxonomyOptions::get_instance();
	}
	...
	public function setSkin( eSmartsElatedClassSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class eSmartsElatedClassSkinManager
 *
...
class eSmartsElatedClassSkinManager {
	...
	/**
	 * @see eSmartsElatedClassSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = 'elated' ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'esmarts' );
...
/**
 * Class eSmartsElatedClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class eSmartsElatedClassSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( esmarts_elated_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.eltdSCIcon = '<?php echo esmarts_elated_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.eltdSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'esmarts' ) ?>';
		</script>
	<?php }
...
/*
   Class: eSmartsElatedClassOptions
...
/*
   Class: eSmartsElatedClassAdminPage
   A class that initializes Elated Admin Page
*/
class eSmartsElatedClassAdminPage implements ieSmartsElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title_label = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new eSmartsElatedClassFieldFactory();
		$this->slug    = $slug;
	...
	public function renderChild( ieSmartsElatedInterfaceRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: eSmartsElatedClassMetaBoxes
...
/*
   Class: eSmartsElatedClassMetaBox
   A class that initializes Elated Meta Box
*/
class eSmartsElatedClassMetaBox implements ieSmartsElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
...
/*
   Class: eSmartsElatedClassTaxonomyOptions
   A class that initializes eSmartsElatedClass Taxonomy Options
*/
class eSmartsElatedClassTaxonomyOptions {
	private static $instance;
...
/*
   Class: eSmartsElatedClassTaxonomyOption
   A class that initializes eSmartsElatedClass Taxonomy Option
*/
class eSmartsElatedClassTaxonomyOption implements ieSmartsElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new eSmartsElatedClassTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
if ( ! function_exists( 'esmarts_elated_init_framework_variable' ) ) {
	function esmarts_elated_init_framework_variable() {
		global $theme_name_php_global_Framework;
		
		$theme_name_php_global_Framework = eSmartsElatedClassFramework::get_instance();
		$eltdSkinManager                 = new eSmartsElatedClassSkinManager();
		$theme_name_php_global_Framework->setSkin( $eltdSkinManager->getSkin() );
	}
	
	add_action( 'esmarts_elated_action_before_options_map', 'esmarts_elated_init_framework_variable' );
}
?>