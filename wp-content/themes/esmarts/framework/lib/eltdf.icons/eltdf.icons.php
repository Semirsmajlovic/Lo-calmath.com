...
/*
  Class: eSmartsElatedClassIconCollections
...
class eSmartsElatedClassIconCollections {
    ...
    private function initIconCollections() {
        $this->addIconCollection('font_awesome', new eSmartsElatedClassIconsFontAwesome("Font Awesome", "fa_icon"));
        $this->addIconCollection('font_elegant', new eSmartsElatedClassIconsFontElegant("Font Elegant", "fe_icon"));
	    $this->addIconCollection('ion_icons', new eSmartsElatedClassIonIcons("Ion Icons", "ion_icon"));
        $this->addIconCollection('linear_icons', new eSmartsElatedClassLinearIcons('Linear Icons', 'linear_icon'));
	    $this->addIconCollection('simple_line_icons', new eSmartsElatedClassSimpleLineIcons('Simple Line Icons', 'simple_line_icon'));
    }
    ...
    public function getIconsMetaBoxOrOption($attributes) {
        ...
        $options = array(
            'font_awesome'      => esc_html__('Font Awesome', 'esmarts'),
            'font_elegant'      => esc_html__('Font Elegant', 'esmarts'),
            'ion_icons'         => esc_html__('Ion Icons', 'esmarts'),
            'simple_line_icons' => esc_html__('Simple Line Icons', 'esmarts')
        );
        if($scope == 'regular') {
            $options = array(
                'font_awesome'      => esc_html__('Font Awesome', 'esmarts'),
                'font_elegant'      => esc_html__('Font Elegant', 'esmarts'),
                'ion_icons'         => esc_html__('Ion Icons', 'esmarts'),
                'linear_icons'      => esc_html__('Linear Icons', 'esmarts'),
                'simple_line_icons' => esc_html__('Simple Line Icons', 'esmarts')
            );
        }
        ...
        foreach ($icon_collections as $collection_key => $collection_object) {
            ...
            $eltd_icon_pack_container = esmarts_elated_add_admin_container(
                array(
                    'parent'			=> $parent,
    ...
    public function getVCParamsArray($iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false) {
        ...
        $iconPackParams = array(
            'type'        => 'dropdown',
            'heading'     => esc_html__('Icon Pack', 'esmarts'),
        ...
        if(is_array($this->iconCollections) && count($this->iconCollections)) {
            foreach($this->iconCollections as $key => $collection) {
                $iconSetParams[] = array(
                    'type'        => 'dropdown',
                    'heading'     => esc_html__('Icon', 'esmarts'),
	...
	public function getIconWidgetParamsArray() {
		...
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'title'   => esc_html__('Icon Pack', 'esmarts'),
			'name'    => 'icon_pack',
			'options' => array(
				'font_awesome'      => esc_html__('Font Awesome', 'esmarts'),
				'font_elegant'      => esc_html__('Font Elegant', 'esmarts'),
    ...
    public function getIconCollectionIcons(ieSmartsElatedInterfaceIconCollection $collection) {
...
if (!function_exists('esmarts_elated_activate_theme_icons')) {
    function esmarts_elated_activate_theme_icons() {
        global $theme_name_php_global_IconCollections;
        $theme_name_php_global_IconCollections = eSmartsElatedClassIconCollections::get_instance();
    }

    add_action('after_setup_theme', 'esmarts_elated_activate_theme_icons');
}