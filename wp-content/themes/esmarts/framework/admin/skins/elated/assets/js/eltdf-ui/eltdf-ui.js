			...
			frame: function() {
				...
				this._frame.on('update', function() {
					...
					jQuery.ajax({
						type: "post",
						url: ajaxurl,
						data: "action=esmarts_elated_gallery_upload_get_images&ids=" + ids,
    ...
    */
	function eltdfInitAjaxForm() {
		...
		$(document).delegate(".eltdf_ajax_form", "submit", function (a) {
			var b = $(this);
			var c = {
				action: "esmarts_elated_save_options"
			};
			jQuery.ajax({
				url: ajaxurl,
				cache: !1,
                            ...
                            if (thisInput.hasClass('my-color-field')) { // if input type is color picker
                                ...
                                if (typeof id !== 'undefined') {
                                    ...
                                    var textareaID = thisInput.attr('id'),
                                        ...
                                        textareaHolder = thisInput.parents('.eltdf-repeater-field').first(),
                                        idForTinyMC = id.substring('eltdf_'.length,id.length+1),
			...
			eltdfImportBtn.on('click', function(e) {
				...
				if (confirm(confirmMessage)) {
					...
					if(import_opt == 'content'){
						for( var i=1; i < 10; i++ ){
							var str;
							if (i < 10) str = 'esmarts_content_0'+i+'.xml';
							else str = 'esmarts_content_'+i+'.xml';
							jQuery.ajax({
								type: 'POST',
								url: ajaxurl,
								data: {
